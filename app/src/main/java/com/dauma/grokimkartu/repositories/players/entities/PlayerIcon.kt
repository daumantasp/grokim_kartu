...
import com.dauma.grokimkartu.repositories.players.PlayersError

class PlayerIcon(private val loader: (onComplete: (Bitmap?, PlayersError?) -> Unit) -> Unit) {
    var icon: Bitmap? = null
        private set
    var status: PlayerIconStatus = PlayerIconStatus.NEED_TO_DOWNLOAD
        private set

    fun loadIfNeeded(onComplete: (Bitmap?, PlayersError?) -> Unit) {
