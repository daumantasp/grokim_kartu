...
class MyThomannsPaginatorImpl(private val thomannsDao: ThomannsDao) : ThomannsPaginator(thomannsDao) {
    override fun loadNextPage(accessToken: String, onComplete: (ThomannsResponse?, ThomannsErrors?) -> Unit) {
        if (isLastLoaded() == false) {
            ...
            thomannsDao.myThomanns(nextPage, pageSize, accessToken) { thomannsResponse, thomannsDaoResponseStatus ->
